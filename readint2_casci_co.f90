...
!        integer :: nz
    ...
    logical :: breit, continue_read
    integer :: idx, read_line_len
...
60  do idx = 1, read_line_len
        read (mdcint, err=20, END=81) i(idx), j(idx), nz(idx), &
            (indk(idx, inz), indl(idx, inz), rklr(idx, inz), rkli(idx, inz), inz=1, nz(idx))
    end do
    goto 82 ! Skip 81 (Skip end read MDCINTNEW)
81  continue_read = .false. ! End read MDCINTNEW
    read_line_len = idx
    !         do inz = 1,nz(idx)
...
12              if (mod(i(idx), 2) == 0) then
                ...
                SignIJ = (-1.0d+00)**mod(i(idx) + j(idx), 2)
                SignKL = (-1.0d+00)**mod(k + l, 2)
! Noda i > jの判定条件を入れ替え(キャッシュミス削減のため)
...
70      End do
    ...
    if (continue_read) then
        Goto 60 ! Continue to read MDCINTNEW
    else
        goto 50 ! Stop to read MDCINTNEW (read MDCINTNEW normally)
    end if

...
! +=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=
    ...
    integer :: mo_offset, mo_start_idx, mo_end_idx, isym_for_syminfo
...
! CMO_new = CMO * congj(U) ! for j and k part (quaternion case)
    do idx_irrep = 1, A
        ...
        if (idx_irrep == 1) then
            ...
            end_isym = nsymrpa
            juck_up_idx = sum(basis_all(1:idx_irrep-1)) + (positronic_mo(idx_irrep) + occ_mo_num(idx_irrep))*basis_ao(idx_irrep)
            num_mo = electronic_mo(idx_irrep) - occ_mo_num(idx_irrep) - vcut_mo_num(idx_irrep)
        end if
        ...
        Do isym = start_isym, end_isym, 2
            nv = count(irpamo(global_sec_start:global_sec_end) == isym .and. &
                (is_kramers_representative(global_sec_start:global_sec_end) .or. &
                 irpamo(kramers_partner(global_sec_start:global_sec_end)) /= isym))
            ...
            Do i = 1, nsec
                i0 = convert_secondary_to_global_idx(i)
                if (irpamo(i0) == isym .and. &
                    (is_kramers_representative(i0) .or. irpamo(kramers_partner(i0)) /= isym)) then
                    nv = nv + 1
                    mosym(nv) = i
                end if
            end do
            ...
            if (idx_irrep == 1) then
                mo_offset = positronic_mo(idx_irrep) + occ_mo_num(idx_irrep)
                mo_start_idx = mo_offset + 1
                mo_end_idx = positronic_mo(idx_irrep) + electronic_mo(idx_irrep) - vcut_mo_num(idx_irrep)
                isym_for_syminfo = isym
            else
                mo_offset = mo(1) + positronic_mo(idx_irrep) + occ_mo_num(idx_irrep)
                mo_start_idx = mo_offset + 1
        ...
        if (B == 2 .or. B == 4) then
            do iao = 1, num_ao
                do imo = 1, num_mo
                    buf_idx = juck_up_idx + (imo - 1)*num_ao + iao
                    BUF(sum(basis_all) + buf_idx) = DIMAG(itrfmo(iao, imo))
                end do
            end do
        end if
        ...
        if (B == 4) then
            do iao = 1, num_ao
                do imo = 1, num_mo
                    buf_idx = juck_up_idx + (imo - 1)*num_ao + iao
                    BUF(2*sum(basis_all) + buf_idx) = DBLE(itrfmo_jk(iao, imo))
                    BUF(3*sum(basis_all) + buf_idx) = DIMAG(itrfmo_jk(iao, imo))
                end do
            end do
        end if
        deallocate (itrfmo)
        ...
        do iao = 1, num_ao
            do imo = 1, num_mo
                ...
                if (B == 1) then
                    ...
                    itrfmo(iao, imo) = DCMPLX(BUF(buf_idx), BUF(sum(basis_all) + buf_idx))
                else if (B == 4) then
                    itrfmo(iao, imo) = DCMPLX(BUF(buf_idx), BUF(sum(basis_all) + buf_idx))
                    itrfmo_jk(iao, imo) = DCMPLX(BUF(2*sum(basis_all) + buf_idx), BUF(3*sum(basis_all) + buf_idx))
                end if
            end do
        end do

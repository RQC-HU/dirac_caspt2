...
def pytest_addoption(parser):
    ...
    parser.addoption("--save", action="store_true", default=False, help="Save the temporary files generated by the tests. [default: False]")
    parser.addoption("--rmtmp", action="store_true", default=False, help="Remove the temporary directories before running the tests. [default: False]")
...
def pytest_collection_modifyitems(config, items):
    ...
    # --rmtmp option is used to remove the temporary directories before running the tests.
    if config.getoption("--rmtmp"):
        print("--rmtmp option was activated. Removing temporary directories before running the tests...")
        test_path = os.path.dirname(os.path.abspath(__file__))
        os.chdir(test_path)
        tmp_dirs = glob.glob("**/tmp", recursive=True)
        for dir in tmp_dirs:

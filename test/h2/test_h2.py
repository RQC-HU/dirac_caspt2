...
def test_h2():
    ...
    p = subprocess.run(
        " ".join(
            [r4dcasci, "&>", output_filename, "&&", r4dcaspt2, "&>>", output_filename]
        ),
        shell=True,
    )
    print(
    ...
    with open(os.path.join(test_path, ref_filename), encoding="utf-8", mode="r") as f:
        grep_str = [
    ...
    with open(
        os.path.join(test_path, output_filename), encoding="utf-8", mode="r"
    ) as f:
    ...
    subprocess.run(" ".join(["mv", output_filename, prev_output]), shell=True)

...
!        thresd = thres
    ...
    if (rank == 0) write(normaloutput,*) 'end after vGmat'
    Call timing(datetmp1, tsectmp1, datetmp0, tsectmp0)
    datetmp1 = datetmp0
    tsectmp1 = tsectmp0
...
! +=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=
    ...
    if (rank == 0) write (normaloutput, *) 'Enter vGmat. Please ignore timer under this line.'
    datetmp1 = date0; datetmp0 = date0
    Call timing(date0, tsec0, datetmp0, tsectmp0)
    v = 0.0d+00
...
20  close (1); goto 100
10  if(rank == 0) write (normaloutput, *) 'error while opening file Gint'; goto 100
...
#endif
    if(rank == 0) write(normaloutput,*) 'end allreduce vGmat'
    Call timing(datetmp1, tsectmp1, datetmp0, tsectmp0)
    datetmp1 = datetmp0
    tsectmp1 = tsectmp0

! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   SUBROUTINE readint2_ord (filename) ! 2 electorn integrals in MDCINT
...
!old        (idum,i0=1,4*nkr),(kr(i0),kr(-1*i0),i0=1,nkr)
 ...
 60     if (realonly) then
             ...
             read (mdcint,ERR=43,END=50) ikr,jkr,nz, &
                  (indk(inz),indl(inz),inz=1,nz), &
                  (rklr(inz), rkli(inz), inz=1,nz)
        endif

!       open(25, file=trim(filename),form ='formatted', status='old', err=10)
...
!        write(*,*) ikr, jkr, kkr, lkr
!        write(*,*)(rklr(inz), inz=1,nz)
!        write(*,'(4I4, E15.5)') ikr, jkr, kkr, lkr, (rklr(inz), inz=1,nz)
...
!                     write(*,'("all ints",4I4,E20.10)')i,j,k,l,rklr(inz)
                     ...
                     If(abs(rklr(inz)) <= 1.0d-12)go to 70
...
!                           write(*,'(4I4,4E20.10)')jtr,itr,k  ,l  ,  SignIJ*rklr(inz),  SignIJ*rkli(inz)
...
!                           write(*,'(4I4,4E20.10)')k  ,l  ,i  ,j  ,         rklr(inz),         rkli(inz)

                        else    ! (22|13)=>(31|22)

                           write(4)ltr,ktr,i  ,j  ,  SignKL*rklr(inz),  SignKL*rkli(inz)
!                           write(*,'(4I4,4E20.10)')ltr,ktr,i  ,j  ,  SignKL*rklr(inz),  SignKL*rkli(inz)
...
!                        elseif(j > i .and. l > k ) then ! (13|23)=>(31|32)
                        elseif(j > i .and. l > k .and. abs(rklr(inz)) >= 1.0d-12) then ! (13|23)=>(31|32)
...
!                        elseif(i < j .and. k > l ) then ! (13|32)=>(31|32)
                        elseif(i < j .and. k > l .and. abs(rklr(inz)) >= 1.0d-12) then ! (13|32)=>(31|32)
...
!                     if(abs(rkli(inz)) > thres) realc = .false.
 ...
 40      write(*,*)'error for reading mdcint 40'
         go to 100
 43      write(*,*)'error for reading mdcint 43'
         go to 100

...
! +=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=
   ...
   PROGRAM r4divo   ! DO IVO CALC ONLY FOR SMALL BASIS SETS
...
!
        ...
        write(*,*)''
        write(*,*)' ENTER R4DIVO PROGRAM written by M. Abe test17 version 2012/10/15'
        write(*,*)''
...
!        nmo = ninact + nact + nsec
      ...
      Call readint2_ivo (filename, nuniq)

...
! +=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=
   ...
   PROGRAM hfc_casci_per  ! Hyperfine coupling constant calculation for perpendicular term at CASCI level
...
! +=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=
   ...
   use four_caspt2_module
        ...
        Implicit NONE
        integer                 :: ii, jj, iatom, iq, i, j, imo, jmo, i0, j0
        logical                 :: test, cutoff
!        real*8                  :: 
        complex*16              :: dens, hfc
...
!
      ...
      Do j = 1, nact
         ...
         hfc = hfc + dens*hfcmo(ii,jj,iq,iatom )
      End do
      End do

...
! +=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=
   ...
   use four_caspt2_module
        ...
        logical :: cutoff
        integer :: j, i, k, syma, symb, isym, sym1, i0, symc
...
!Iwamuro think
!               syma = MULTB(irpmo(jx), isym)
!               symb = MULTB2(irpmo(jz), nsymrpa + 1)
!               symc = MULTB(irpmo(jy), symb)      
!Iwamuro modify
!                write(*,*)"syma1",syma
!Iwamuro modify
                If(nsymrpa==1.or.(nsymrpa/=1.and.(syma == symc))) then  
...
! +=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=
   ...
   SUBROUTINE bCmat (dimn, sc, indsym, bc) ! Assume C1 molecule, overlap matrix B in space C
!
! Indices are restricted as t > v, x > z
! So the dimension of (xyz) is (norb**3+norb**2)/2
!
...
!           write(*,'("TYPE 2  ",4I4,2E20.10)')i,j,k,l,cint2
!
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!  effh(a,p) =  hap + Siguma_k(is oqqupied)[(ap|kk)-(ak|kp)] - Siguma_w(aw|wp)
!                                          ========
...
!           write(*,'("TYPE 3  ",4I4,2E20.10)')i,j,k,l,cint2
!
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!  effh(a,p) =  hap + Siguma_k(is oqqupied)[(ap|kk)-(ak|kp)] - Siguma_w(aw|wp)
!                                                  =========
